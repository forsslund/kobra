...
for a_project in projects_list:
  ...
  for ex_dir in a_project[1]:
    ...
    if os.path.isdir( examples_dir ):
      ...
      source_dir = a_project[4]
      if len( a_project[1] ) > 1:
        path_to_append = "/" + ex_dir.split(a_project[4] + "/")[-1]
        output_dir = output_dir + path_to_append
        source_dir = source_dir + path_to_append
      ...
      for root, dirs, files in os.walk(examples_dir):
        ...
        for temp_file in files:
          ...
          if( temp_file.endswith(".x3d") ):
            ...
            anchor_name = file_name_no_end
            while anchor_name in anchor_list:
              anchor_name = anchor_name + "0"
            anchor_list.append(anchor_name)
            examples_doxygen_page = examples_doxygen_page + """/// - \\anchor %(anchor_name)s_x3d
    ///   <a href="%(rel_path)s%(stripped_file)s">%(tf)s</a>
    ///   ( <a href="%(source_dir)s/%(tf)s.html">Source</a> )
    """ % { 'fn_ne' : file_name_no_end, 'rel_path' : a_project[5],'tf': temp_file, 'source_dir' : source_dir, 'stripped_file' : file_path_stripped, 'anchor_name' : anchor_name }
