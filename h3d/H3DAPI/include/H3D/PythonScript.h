...
namespace H3D {
  ...
  struct H3DAPI_API PythonScript : public H3DScriptNode {
    ...
    /// Returns a borrowed reference to the PyModule object use by the 
    /// PythonScript node.
    inline PyObject *getPythonModule() {
      return static_cast< PyObject *>(module);
    }
  ...
  private:
    ...
    void * module_dict;
    static int argc;
    // Set the argv parameter in Python from the PythonScript::argv or PythonScript::w_argv
    // depending on which is in use.
    static void pythonSetargv();

    // argv or w_argv is the program argv parameter. Only one will be set to non-NULL. Which 
    // on depends if the main function is the wide chars version or not.
    static char **argv;
    static wchar_t **w_argv;
    string module_name;
    static bool python_h3d_initialized;
 };
}
