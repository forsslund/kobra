...
namespace FalconDeviceInternals {
  ...
  FIELDDB_ELEMENT( FalconDevice, usedDriver, OUTPUT_ONLY )
}
...
void FalconDevice::initialize() {
  ...
  usedDriver->setValue( "NIFALCON", id );
#else
#ifdef HAVE_FALCONAPI
    if( device_name != "" ) {
...
#ifdef HAVE_FALCONAPI
   HAPI::FalconHapticsDevice *fd = 
    dynamic_cast< HAPI::FalconHapticsDevice * >(hapi_device.get() );
  if( e == HAPI::HAPIHapticsDevice::SUCCESS && fd ) {
    deviceModelType->setValue( fd->getDeviceModel(), id );
    HAPI::Vec3 max, min;
    fd->getWorkspaceDimensions( min, max );
