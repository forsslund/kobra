...
void NoiseTexture::SFImage::update() {
  ...
  for( y = 0; y < _height; ++y ) {
    for( x = 0; x < _width; ++x) {
      ...
      if( !simplexNoise ) {
		  // Classical Perlin noise is not implemented
		  // We set the perlinValue to 0 to avoid having an undefined value, which will result in an uniform grey texture
		  perlinValue = H3DFloat( 0 );
      } else {
        if( _tileable ) {
