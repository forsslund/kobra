...
void TransferFunctionTexture::SFImage::update() {
  ...
  bool red_well_defined = true;
  bool green_well_defined = true;
  bool blue_well_defined = true;
  bool alpha_well_defined = true;

  if( red_function && red_function->nrInputValues() != 1 ) {
    H3DUtil::Console( LogLevel::Warning ) << "TransferFunctionTextures red function must take exactly 1 input, got " << red_function->nrInputValues() << ". "
      << "Using default identity function instead." 
      << endl;
    red_well_defined = false;
  }
  if( green_function && green_function->nrInputValues() != 1 ) {
    H3DUtil::Console( LogLevel::Warning ) << "TransferFunctionTextures green function must take exactly 1 input, got " << green_function->nrInputValues() << ". "
      << "Using default identity function instead." 
      << endl;
    green_well_defined = false;
  }
  if( blue_function && blue_function->nrInputValues() != 1 ) {
    H3DUtil::Console( LogLevel::Warning ) << "TransferFunctionTextures blue function must take exactly 1 input, got " << blue_function->nrInputValues() << ". "
      << "Using default identity function instead."
      << endl;
    blue_well_defined = false;
  }
  if( alpha_function && alpha_function->nrInputValues() != 1 ) {
    H3DUtil::Console( LogLevel::Warning ) << "TransferFunctionTextures alpha function must take exactly 1 input, got " << alpha_function->nrInputValues() << ". "
      << "Using default identity function instead."
      << endl;
    alpha_well_defined = false;
  }
  
  unsigned char lut[256*4];
  for( int i = 0 ; i < 256 ; ++i ){
    lut[i*4  ] = (unsigned char) (red_function && red_well_defined ?
                                  red_function->evaluate( i / 255.0 ) * 255 :
                                  i ) ;
    lut[i*4+1] = (unsigned char) (green_function && green_well_defined ?
                                  green_function->evaluate( i / 255.0 ) * 255: 
                                  i );
    lut[i*4+2] = (unsigned char) (blue_function && blue_well_defined ?
                                  blue_function->evaluate( i / 255.0 ) * 255 : 
                                  i );
    lut[i*4+3] = (unsigned char) (alpha_function && alpha_well_defined ?
