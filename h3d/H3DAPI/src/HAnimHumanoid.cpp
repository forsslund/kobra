...
HAnimHumanoid::HAnimHumanoid(  Inst< SFNode         > _metadata    ,
  ...
  jointMatrixChanged->setOwner( this );
  jointMatrixChanged->setName( "jointMatrixChanged" );

  renderMode->route( displayList );
  renderMode->route( jointMatrixChanged );
 }
...
void HAnimHumanoid::traverseSG( TraverseInfo &ti ) {
  ...
  if( !jointMatrixChanged->isUpToDate() ) {
    jointMatrixChanged->upToDate();
...
void HAnimHumanoid::MFJoint::onAdd( Node *n ) {
  ...
  if( n ) {
    if( HAnimJoint *joint = dynamic_cast< HAnimJoint *>( n ) ) {
      joint->accumulatedJointMatrix->route( humanoid->jointMatrixChanged );
      joint->displacers->route( humanoid->jointMatrixChanged );
...
void HAnimHumanoid::MFJoint::onRemove( Node *n ) {
  ...
  if( HAnimJoint *joint = dynamic_cast< HAnimJoint *>( n ) ) {
    joint->accumulatedJointMatrix->unroute( humanoid->jointMatrixChanged );
    joint->displacers->unroute( humanoid->jointMatrixChanged );
