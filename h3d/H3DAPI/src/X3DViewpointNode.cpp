...
void X3DViewpointNode::setupViewMatrix( EyeMode eye_mode,
  ...
  Vec3f axis = vp_orientation.axis;
  axis.normalizeSafe();
  Matrix4f result = Matrix4f( Rotation( vp_orientation.axis, -vp_orientation.angle ) ) * Matrix4f( -vp_position ) * vp_inv_m;
  ...
  if( eye_mode != MONO ) {
    ...
    if( eye_mode == LEFT_EYE ) {
      result = Matrix4f( -left_eye ) * result;
  ...
  // Transpose to convert to column major
  glMultMatrixf( &result.transpose()[0][0] );
