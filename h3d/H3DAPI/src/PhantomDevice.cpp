...
H3DHapticsDevice::ErrorCode H3D::PhantomDevice::enableDevice() {
  if( started_scheduler ) {
    return H3DHapticsDevice::enableDevice();
  } else {
    return HAPI::HAPIHapticsDevice::NOT_ENABLED;
  }
}
...
#ifdef HAVE_OPENHAPTICS
  if( hapi_device.get() ) {
    if( !started_scheduler && (nr_initialized_devices > 0) ) {
        HAPI::PhantomHapticsDevice::startScheduler();
        started_scheduler = HAPI::PhantomHapticsDevice::isSchedulerStarted();
    }
    if( started_scheduler ) {
      enableDevice(); // Now that we know we have a scheduler we can finally start the device.
    }
  }
#endif
}
