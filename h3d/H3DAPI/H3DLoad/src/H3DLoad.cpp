...
int main(int argc, char* argv[]) {
...
#ifdef HAVE_PYTHON
  PythonScript::initPythonHome( argv[0] );
  ...
  help_message += "    --screen=<W>x<H>    Set width and height for the opengl rendering area.\n";
  help_message += "                        Note that the operating system might limit the size\n";
  help_message += "                        range due to captions and borders.\n";
  help_message += "\n";
  ...
  help_message += "\n";
#ifdef HAVE_NVAPI
  help_message += "    --nvsettings=<file> Path to a file that contains a NvidiaSettings\n";
  help_message += "                        node as it top most node.\n";
  help_message += "\n";
#endif
...
#ifdef HAVE_NVAPI
      else if( !strncmp(argv[i]+2,"nvsettings=", strlen("nvsettings=")) ) {
        Scene::nvidia_graphics_options_file_url = strstr(argv[i],"=")+1;
      }
#endif
      else {
