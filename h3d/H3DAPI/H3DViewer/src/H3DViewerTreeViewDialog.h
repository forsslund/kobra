...
public:
  ...
  // Callback for profiling checkbox
  void OnStopUpdatingCheckbox( wxCommandEvent& event );
  ...
  // Update a current tree the way it should be updated when "Stop updating" is checked.
  // Returns trye if the contained H3D.Node in tree_id and all of its children have
  // been destructed.
  bool updateNodeTreeWhenUpdateStopped( wxTreeItemId tree_id );
  ...
  // Update the treeview_node_count_map member with new data based on current tree view.
  void updateNodeCountMap( wxTreeItemId tree_id );

  // map from Node to nr of times that node is used as a tree item in the tree view.
  std::map< Node *, unsigned int > treeview_node_count_map;
