...
public:
  ...
  // There is currently no way to just hide the menu bar using wxWidgets functions.
  // However, menu works fine in fullscreen even though it is not shown.
  // Sometimes there are weird cases in which it is useful to have a windowed mode
  // with a size but no menu bar shown. The menu shortcuts should work however
  // which is the reason for using this hack instead of setting the active menu bar
  // to NULL.
  void hideMenuBarHack();
