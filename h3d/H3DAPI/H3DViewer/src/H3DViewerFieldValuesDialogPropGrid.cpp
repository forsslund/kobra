...
H3DViewerFieldValuesPanelPropGrid::H3DViewerFieldValuesPanelPropGrid( wxWindow* parent, 
  ...
  AUTOREF_DEBUG_NAME( displayed_node, "H3DViewerFieldValuesDialogPropGrid::displayed_node")
...
void H3DViewerFieldValuesPanelPropGrid::OnIdle( wxIdleEvent& /*event*/ ) {
  ...
  if( now - last_fields_update > 0.25 ) {
    ...
    if( mode == 0 ) {
#ifdef H3D_REFERENCE_COUNT_DEBUG
    H3DViewerFieldValuesDialogPropGrid *parent = dynamic_cast<H3DViewerFieldValuesDialogPropGrid *>( GetParent() );
    if (parent) {
      parent->displayFieldsFromNode( displayed_node.get() );
    }
#else
...
void H3DViewerFieldValuesPanelPropGrid::setupProperty( wxPropertyGrid *FieldValuesGrid,
  ...
  if(p) {
    // Avoid adding a property if it already exists. This is because there are some fields which
    // have alias names, and the same property name can not be used twice.
    wxPGProperty *values_grid_root = FieldValuesGrid->GetRoot();
    for( unsigned int i = 0; i < values_grid_root->GetChildCount(); ++i ) {
      wxPGProperty *category = values_grid_root->Item( i );
      for( unsigned int j = 0; j < category->GetChildCount(); ++j ) {
        wxPGProperty *field_value_item = category->Item( j );
        if( field_value_item->GetName().Cmp( p->GetName() ) == 0 ) {
          delete p;
          return;
        }
      }
...
#if ( defined( __WXMAC__ ) && ( wxMAJOR_VERSION >= 3 ) && ( wxMINOR_VERSION >= 1 ) && ( wxRELEASE_NUMBER >= 1 ) )
