...
class DistanceJoint ( Joint ):
    """ A visual representation of a DistanceJoint"""

    class DetailsString ( TypedField ( MFString, SFFloat ) ):
        """ Field to convert output fields to strings"""
        def update ( self, event ):
            r= self.getRoutesIn()
            return [ "distance: " + str(r[0].getValue()) ]

    class AnchorPoint ( TypedField( SFVec3f, SFVec3f, SFVec3f ) ):
        """ Calculate the AnchorPoint for a DistanceJoint."""
        def update( self, event ):
          routes_in = self.getRoutesIn()
          if len(routes_in) >= 2:
            return ( routes_in[0].getValue() + routes_in[1].getValue() ) / 2
          else:
            return Vec3f( 0, 0, 0 )

    def __init__ ( self, joint ):
        super ( DistanceJoint, self ).__init__ ( joint )
        ...
        self.anchorPoint = DistanceJoint.AnchorPoint()
        
        # Set up routes to draw line 1
        if joint.body1.getValue():
            self.drawLine1= DrawLine ()
            self.anchorPoint.route ( self.drawLine1 )
            joint.body1.getValue().position.route ( self.anchorPoint )
            joint.body1.getValue().position.route ( self.drawLine1 )
            self.drawLine1.route ( dnLines["Line1"].coord.getValue().point )
            dnLines["Line1"].vertexCount.push_back ( 2 )
        ...
        if joint.body2.getValue():
            self.drawLine2= DrawLine ()
            self.anchorPoint.route ( self.drawLine2 )
            joint.body2.getValue().position.route ( self.anchorPoint )
            joint.body2.getValue().position.route ( self.drawLine2 )
            self.drawLine2.route ( dnLines["Line2"].coord.getValue().point )
            dnLines["Line2"].vertexCount.push_back ( 2 )
        
        self.anchorPoint.route ( self.dn["Title_T"].translation )
        self.anchorPoint.route ( self.dn["AnchorPoint_T"].translation )

    def showDetails ( self ):
        """ Show output field details"""
        self.detailsString= DistanceJoint.DetailsString ()
        self.joint.distance.route ( self.detailsString )
        super ( DistanceJoint, self ).showDetails ()
...
class UpdateJoints ( AutoUpdate ( MFNode ) ):
    ...
    def update ( self, event ):
        ...
        for j in joints:
            ...
            if j.getTypeName()=="DistanceJoint":
                joint= DistanceJoint( j )
