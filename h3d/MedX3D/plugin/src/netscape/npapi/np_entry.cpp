...
NPError OSCALL NP_Shutdown()
{
    ...
    cerr << "NP_Shutdown" << endl;
#endif
  NS_PluginShutdown();
  return NPERR_NO_ERROR;
}

extern "C" void NP_ShutdownMain()
{
#ifdef PRINT_ERR_FILE
    cerr << "NP_ShutdownMain" << endl;
#endif
  NS_PluginShutdown();
  }
...
{
    if(aNPNFuncs == NULL) {
        cerr << "func table error" << endl;
        return NPERR_INVALID_FUNCTABLE_ERROR;
    }

    if(HIBYTE(aNPNFuncs->version) > NP_VERSION_MAJOR) {
        cerr << "version error: " << HIBYTE(aNPNFuncs->version) << " " << NP_VERSION_MAJOR << endl;
        return NPERR_INCOMPATIBLE_VERSION_ERROR;
    }

    if(aNPNFuncs->size < sizeof(NPNetscapeFuncs)) {
        cerr << "size error:"  << aNPNFuncs->size << " " << sizeof(NPNetscapeFuncs) << endl;
    return NPERR_INVALID_FUNCTABLE_ERROR;
...
#ifdef PRINT_ERR_FILE
    cerr << "NP_GetEntryPoints" << endl;
#endif
    return fillPluginFunctionTable(aNPPFuncs);
}

char * NP_GetMIMEDescription(void) {
    ofstream os( "/home/daniel/test.tt" );
    os << "MIME: " << endl;
    os << NPP_GetMIMEDescription();
    os.close();
...
int main(NPNetscapeFuncs *browserFuncs, NPPluginFuncs *pluginFuncs, NPP_ShutdownProcPtr *shutdown ) {
    ...
    cerr << "main()" << endl;
#endif
    NPError res = NP_Initialize( browserFuncs );
    if( res != NPERR_NO_ERROR ) {
        return res;
    }

    res = NP_GetEntryPoints( pluginFuncs );
    if( res != NPERR_NO_ERROR ) {
        return res;
    }

    *shutdown = (NPP_ShutdownProcPtr) NP_ShutdownMain;
    return NPERR_NO_ERROR;
