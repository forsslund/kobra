    ...
    for temp_file in files:
      if( not temp_file.startswith("X3D") and not temp_file.startswith("H3D") and temp_file.endswith(".h") ):
        ...
        if name_count > 0:
          ...
          if( name_count > 1 ):
            print "name_count is above 1, there should not be two files with the same name in the UI/x3d directory"

          fhandle = open( join( headers_dir, temp_file ), "rb" )
          code = fhandle.read()
          fhandle.close()

          new_string1 = """  ///   - <a href="../../x3d/%s.x3d">%s.x3d</a>""" %( file_name_no_end, file_name_no_end )
          new_string2 = """  ///     ( <a href="x3d/%s.x3d.html">""" % ( file_name_no_end )
  ...
  for root, dirs, files in os.walk(headers_dir):
    ...
    for temp_file in files:
      if( not temp_file.startswith("X3D") and not temp_file.startswith("H3D") and temp_file.endswith(".h") ):
        ...
        examples_to_remove = []
        for i in examples:
          find_string1 = """  ///   - <a href="../../../../MedX3D/x3d/%s.x3d">%s.x3d</a>""" % (i, i)
          find_string2 = """  ///     ( <a href="x3d/%s.x3d.html">""" % (i)
