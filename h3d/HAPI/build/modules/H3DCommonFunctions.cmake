...
# addCommonH3DMSVCCompileFlags( <compile_flags_container> [<default_compiler_warning_level>] )
...
# <default_compiler_warning_level> - Set the warning level to the compile default (i.e. do not add any flags).
function( addCommonH3DMSVCCompileFlags compile_flags_container )
  if( MSVC )
    set( default_compiler_warning_level OFF )
    if( ${ARGC} GREATER 1 )
      list( GET ARGN 0 default_compiler_warning_level )
    endif()
    ...
    if( NOT default_compiler_warning_level )
      set( compile_flags_container_internal "${compile_flags_container_internal} /W4" )
    endif()
...
# addCommonH3DGNUCompileFlags( <compile_flags_container> [<force_deprecated_declarations> [<default_compiler_warning_level>]] )
#
# <compile_flags_container> - Compile flags will be added here
# <force_deprecated_declarations> - Force deprecated declarations.
# <default_compiler_warning_level> - Set the warning level to the compile default (i.e. do not add any flags).
...
function( addCommonH3DCompileFlags compile_flags_container )
  ...
  set( compile_flags_container_internal )
  addCommonH3DMSVCCompileFlags( compile_flags_container_internal ${default_compiler_warning_level} )
...
function( embedShaderLib shader_src_files header_template header_output )
  ...
  foreach( shader_file ${shader_src_files} )
	...
	# To avoid the maximum string literal constraints on MSVC we break the string up into a series
	# of maximally sized literals and concatinate them into a std::string.
	set( offset 0 )
	set( limit 16384 ) # Maximum multiline string literal size (bytes)

	# Read everything
    file( READ ${CMAKE_CURRENT_BINARY_DIR}/${shader_file} source_code )
	string( LENGTH "${source_code}" string_length )

	# Output the first chunk
	string( SUBSTRING "${source_code}" ${offset} ${limit} sub_string )
    set( shader_str_declarations
      "${shader_str_declarations}\nconst std::string ${string_name} =\nstd::string( R\"GLSL(${sub_string})GLSL\" )" )
	math( EXPR offset "${offset}+${limit}" )

	# Loop over and output the remaining chunks (if any)
	while( offset LESS string_length )
	  string( SUBSTRING "${source_code}" ${offset} ${limit} sub_string )
	  set( shader_str_declarations
        "${shader_str_declarations}\n + std::string( R\"GLSL(${sub_string})GLSL\" )" )
      math( EXPR offset "${offset}+${limit}" )
